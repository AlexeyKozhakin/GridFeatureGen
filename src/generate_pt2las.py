...
if __name__ == "__main__":
    ...
    print('inp_las:',input_las_dir)
    print('input_pt:',input_pt_dir)


    las_files = [f for f in os.listdir(input_las_dir) if f.endswith('.las')]
    las_file = os.path.join(input_las_dir, las_files[0])
    ...
    xx, yy = np.meshgrid(x_grid, y_grid)  # Координаты сетки
    grid_points = np.c_[xx.ravel(), yy.ravel()]  # (M*M, 2) - координаты точек сетки
    ...
    distances, indices = tree.query(points[:, :2])  # Ищем соседей только для (x, y)

    # Присвоение классов точкам
    classes = pt[:,:,6].T.reshape(-1)[indices]  # Классы из сетки для ближайших соседей
